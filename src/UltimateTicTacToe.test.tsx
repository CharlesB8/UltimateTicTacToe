...
test("X can win the ultimate board", () => {
    ...
    let subBoardsSquares: HTMLElement[][];
    subBoardsSquares = [];

    // for (let i = 0; i < subBoards.length; i++) {
    //     subBoardsSquares.push(
    //         within(subBoards[i]).getAllByTestId('square')
    //     )
    // }

    subBoards.forEach((subBoard, index) => {
        subBoardsSquares.push(within(subBoards[index]).getAllByTestId('square'))
    })
